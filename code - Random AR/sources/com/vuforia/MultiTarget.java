...
public class MultiTarget extends ObjectTarget {
    ...
    public Trackable getPart(String str) {
        ...
        str = new Trackable(MultiTarget_getPart, false);
        if (str.isOfType(ImageTarget.getClassType())) {
            return new ImageTarget(MultiTarget_getPart, false);
        }
        if (str.isOfType(CylinderTarget.getClassType())) {
        ...
        if (str.isOfType(VuMarkTarget.getClassType())) {
            return new VuMarkTarget(MultiTarget_getPart, false);
        }
        if (str.isOfType(VuMarkTemplate.getClassType())) {
            return new VuMarkTemplate(MultiTarget_getPart, false);
        }
        if (str.isOfType(ModelTarget.getClassType())) {
            return new ModelTarget(MultiTarget_getPart, false);
        }
        if (str.isOfType(ObjectTarget.getClassType())) {
            return new ObjectTarget(MultiTarget_getPart, false);
        }
        if (str.isOfType(Anchor.getClassType())) {
            return new Anchor(MultiTarget_getPart, false);
        }
        if (str.isOfType(DeviceTrackable.getClassType()) != null) {
