...
public class DataSet {
    ...
    public Trackable createTrackable(TrackableSource trackableSource) {
        ...
        trackableSource = new Trackable(DataSet_createTrackable, false);
        if (trackableSource.isOfType(ImageTarget.getClassType())) {
            return new ImageTarget(DataSet_createTrackable, false);
        }
        if (trackableSource.isOfType(CylinderTarget.getClassType())) {
            return new CylinderTarget(DataSet_createTrackable, false);
        }
        if (trackableSource.isOfType(MultiTarget.getClassType())) {
            return new MultiTarget(DataSet_createTrackable, false);
        }
        if (trackableSource.isOfType(VuMarkTarget.getClassType())) {
            return new VuMarkTarget(DataSet_createTrackable, false);
        }
        if (trackableSource.isOfType(VuMarkTemplate.getClassType())) {
            return new VuMarkTemplate(DataSet_createTrackable, false);
        }
        if (trackableSource.isOfType(ModelTarget.getClassType())) {
            return new ModelTarget(DataSet_createTrackable, false);
        }
        if (trackableSource.isOfType(ObjectTarget.getClassType())) {
            return new ObjectTarget(DataSet_createTrackable, false);
        }
        if (trackableSource.isOfType(Anchor.getClassType())) {
            return new Anchor(DataSet_createTrackable, false);
        }
        if (trackableSource.isOfType(DeviceTrackable.getClassType()) != null) {
