...
public class TargetSearchResultList implements Cloneable, Iterable<TargetSearchResult> {
    ...
    public TargetSearchResult at(int i) {
        ...
        if (TargetSearchResultList_at == 0) {
            return null;
        }
        TargetSearchResult targetSearchResult = new TargetSearchResult(TargetSearchResultList_at, false);
        if (targetSearchResult.isOfType(CloudRecoSearchResult.getClassType())) {
            return new CloudRecoSearchResult(TargetSearchResultList_at, false);
        }
        if (targetSearchResult.isOfType(ModelRecoSearchResult.getClassType())) {
            return new ModelRecoSearchResult(TargetSearchResultList_at, false);
        }
        return null;
    }
